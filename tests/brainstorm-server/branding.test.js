/**
 * Tests for the visual companion's Governed-Superpowers/Prime Radiant branding.
 */
...
const ASSET_URL = 'https://primeradiant.com/brand/governed-superpowers-visual-brainstorming-logo.png';
...
function createPackagedServerFixture(version) {
  const root = fs.mkdtempSync(path.join('/tmp', 'governed-superpowers-packaged-server-'));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'governed-superpowers', version }, null, 2)
  );
  return {
    root,
...
function assertBrandedWithLogo(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Governed-Superpowers v${version}`),
    'branding text should include dynamic package version'
  );
  assert(
    !html.includes(`Governed-Superpowers v${version} by`),
    'branding text should not include "by" when the logo is visible'
  );
  assert(
    /<img class="brand-logo"[^>]*>\s*<span class="brand-copy">Governed-Superpowers v/.test(html),
    'visible logo should appear before the Governed-Superpowers version text'
  );
  assert(
...
function assertBrandedFallbackText(html, version = PACKAGE_VERSION) {
  assert(
    html.includes(`Prime Radiant Governed-Superpowers v${version}`),
    'disabled telemetry should keep plain text Prime Radiant/Governed-Superpowers branding'
  );
}
  ...
  await test('packaged Codex plugin reads version from .codex-plugin manifest', async () => {
    ...
    try {
      await withServer({ port, dir, serverPath: fixture.serverPath }, async () => {
        ...
        assert(!html.includes('Governed-Superpowers vunknown'), 'packaged plugin should not fall back to unknown version');
      });
    } finally {
