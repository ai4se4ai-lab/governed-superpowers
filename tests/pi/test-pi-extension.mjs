...
const extensionPath = resolve(repoRoot, '.pi/extensions/governed-superpowers.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-governed-superpowers/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'governed-superpowers');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/governed-superpowers.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have governed-superpowers/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have governed-superpowers/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have governed-superpowers/);
