#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the governed-superpowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/governed-superpowers.js"
...
# Test 3: Check using-governed-superpowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-governed-superpowers skill (required for bootstrap)..."
if [ -f "$SUPERPOWERS_SKILLS_DIR/using-governed-superpowers/SKILL.md" ]; then
    echo "  [PASS] using-governed-superpowers skill exists"
else
    echo "  [FAIL] using-governed-superpowers skill not found (required for bootstrap)"
    exit 1
fi
...
if grep -q 'configDir}/skills/governed-superpowers/' "$SUPERPOWERS_PLUGIN_FILE"; then
