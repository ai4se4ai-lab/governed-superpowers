...
# 1. Create in governed-superpowers location (lowest priority)
mkdir -p "$SUPERPOWERS_SKILLS_DIR/priority-test"
cat > "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Governed-Superpowers version of priority test skill
---
# Priority Test Skill (Governed-Superpowers Version)
...
if [ -f "$SUPERPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Governed-Superpowers version exists"
else
    echo "  [FAIL] Governed-Superpowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled governed-superpowers priority
echo ""
echo "Test 2: Documenting personal vs governed-superpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled governed-superpowers version instead of personal version"

# Test 3: Document project vs bundled governed-superpowers priority
echo ""
echo "Test 3: Documenting project vs personal/governed-superpowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled governed-superpowers version instead of project version"

# Test 4: Test a non-colliding bundled governed-superpowers skill is still available
echo ""
echo "Test 4: Testing non-colliding governed-superpowers skill remains available..."

mkdir -p "$SUPERPOWERS_SKILLS_DIR/governed-superpowers-only-test"
cat > "$SUPERPOWERS_SKILLS_DIR/governed-superpowers-only-test/SKILL.md" <<'EOF'
---
name: governed-superpowers-only-test
description: Governed-Superpowers-only priority test skill
---
# Governed-Superpowers Only Test Skill

PRIORITY_MARKER_SUPERPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"governed-superpowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERPOWERS_ONLY_VERSION" "Non-colliding governed-superpowers skill is still registered"
