...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/governed-superpowers-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"

# Create project directory (conversation is cwd-based)
PROJECT_DIR="$OUTPUT_DIR/project"
mkdir -p "$PROJECT_DIR/docs/governed-superpowers/plans"
...
cat > "$PROJECT_DIR/docs/governed-superpowers/plans/auth-system.md" << 'EOF'
...
claude -p "Good analysis. I've already written the plan to docs/governed-superpowers/plans/auth-system.md. Now I'm ready to implement. What are my options for execution?" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
