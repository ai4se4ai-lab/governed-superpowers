...
EOF
    ...
    cat > "$repo/assets/governed-superpowers-small.svg" <<'EOF'
...
EOF
    ...
    git -C "$repo" add \
        ...
        assets/app-icon.png \
        assets/governed-superpowers-small.svg \
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/governed-superpowers/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/governed-superpowers/.fixture-keep"
    cat > "$repo/plugins/governed-superpowers/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/governed-superpowers/.fixture-keep
    git -C "$repo" add plugins/governed-superpowers/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/governed-superpowers/skills/example/agents"

    cat > "$repo/plugins/governed-superpowers/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/governed-superpowers/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/governed-superpowers/.codex-plugin" \
        "$repo/plugins/governed-superpowers/.private-journal" \
        "$repo/plugins/governed-superpowers/assets" \
        "$repo/plugins/governed-superpowers/hooks" \
        "$repo/plugins/governed-superpowers/skills/example/agents" \
        "$repo/plugins/governed-superpowers/skills/example"

    cat > "$repo/plugins/governed-superpowers/.codex-plugin/plugin.json" <<EOF
{
  "name": "governed-superpowers",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/governed-superpowers/assets/governed-superpowers-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/governed-superpowers/assets/app-icon.png"

    cat > "$repo/plugins/governed-superpowers/hooks/hooks-codex.json" <<'EOF'
{
  "hooks": {
    "SessionStart": [
      {
...
EOF
    ...
    cat > "$repo/plugins/governed-superpowers/hooks/session-start" <<'EOF'
#!/usr/bin/env sh
echo "session-start fixture"
EOF
    cat > "$repo/plugins/governed-superpowers/hooks/session-start-codex" <<'EOF'
#!/usr/bin/env sh
echo "session-start-codex fixture"
EOF

    cat > "$repo/plugins/governed-superpowers/hooks/run-hook.cmd" <<'EOF'
@echo off
echo run-hook fixture
EOF
    chmod +x "$repo/plugins/governed-superpowers/hooks/session-start" "$repo/plugins/governed-superpowers/hooks/session-start-codex" "$repo/plugins/governed-superpowers/hooks/run-hook.cmd"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/governed-superpowers/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/governed-superpowers/.codex-plugin/plugin.json \
        plugins/governed-superpowers/assets/app-icon.png \
        plugins/governed-superpowers/assets/governed-superpowers-small.svg \
        plugins/governed-superpowers/hooks/hooks-codex.json \
        plugins/governed-superpowers/hooks/run-hook.cmd \
        plugins/governed-superpowers/hooks/session-start \
        plugins/governed-superpowers/hooks/session-start-codex \
        plugins/governed-superpowers/skills/example/agents/openai.yaml \
        plugins/governed-superpowers/skills/example/SKILL.md \
        plugins/governed-superpowers/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/governed-superpowers/.kimi-plugin" \
        "$repo/plugins/governed-superpowers/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/governed-superpowers/.fixture-keep"
    printf '{"name":"stale-kimi"}\n' > "$repo/plugins/governed-superpowers/.kimi-plugin/plugin.json"
    printf 'stale ignored leak\n' > "$repo/plugins/governed-superpowers/.private-journal/leak.txt"
    git -C "$repo" add \
        plugins/governed-superpowers/.fixture-keep \
        plugins/governed-superpowers/.kimi-plugin/plugin.json
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/governed-superpowers/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/governed-superpowers/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/governed-superpowers-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (governed-superpowers-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/governed-superpowers-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/governed-superpowers/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/governed-superpowers" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/governed-superpowers-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/governed-superpowers'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/governed-superpowers-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/governed-superpowers-*" "Clean no-op local apply does not create sync branch in destination checkout"
