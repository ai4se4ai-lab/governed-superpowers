...
assert_equal(marketplace.get("name"), "governed-superpowers-dev", "marketplace name")
assert_equal(
    marketplace.get("interface", {}).get("displayName"),
    "Governed-Superpowers Dev",
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "governed-superpowers"]
assert_equal(len(matching_plugins), 1, "governed-superpowers plugin entry count")
