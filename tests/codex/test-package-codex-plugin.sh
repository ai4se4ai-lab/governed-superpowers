...
archive="$TEST_ROOT/governed-superpowers"
tar_archive="$TEST_ROOT/governed-superpowers.tar.gz"
...
unexpected_pattern='(^governed-superpowers/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)'
...
assert_contains "$archive_paths" "assets/governed-superpowers-small.svg" "archive includes composer icon"
...
assert_equals "$manifest_summary" "governed-superpowers	$expected_version	./skills/	$source_hooks" "archive manifest preserves source hooks"
...
archive_from_tar_source="$TEST_ROOT/governed-superpowers-from-tar-source.zip"
archive_from_zip_source="$TEST_ROOT/governed-superpowers-from-zip-source.zip"
(
  cd "$metadata_source"
