# Governed-Superpowers Release Notes
...
The `using-governed-superpowers` bootstrap is injected into every session, so its size is paid for constantly. This release trims it and the per-harness references it points to, without dropping behavior-shaping content.

- **Compressed the `using-governed-superpowers` bootstrap.** Replaced the graphviz skill-flow diagram with the prose it encoded, folded the standalone Instruction-Priority section into User Instructions, dropped the per-platform "How to Access Skills" walkthrough, and trimmed the Platform Adaptation pointer to the harnesses that still ship a reference file. The full Red Flags rationalization table and the user-instruction precedence rules are unchanged.
...
- **Version display in the brainstorm companion** — packaged Codex plugins ship without a root `package.json`, so the visual companion reported its version as "unknown". `readGoverned-SuperpowersVersion()` now falls back to `.codex-plugin/plugin.json` when `package.json` is absent.
...
Governed-Superpowers 6.0 is a big release. The headline is a rewrite of how `subagent-driven-development` reviews each task — cheaper, stricter, and harder to game. 
...
- **The legacy global worktree directory is gone.** `using-git-worktrees` and `finishing-a-development-branch` no longer use `~/.config/governed-superpowers/worktrees/`. Worktrees now land in the project — an existing `.worktrees/` or `worktrees/` if you have one, otherwise a fresh `.worktrees/` — unless you say otherwise.

### New Harness Support

Governed-Superpowers now runs on three more harnesses. Each ships its own bootstrap, a tool-mapping reference, and tests, and each gets its own install section in the README.
...
- **Pi** — a session-start extension that registers the skills and injects the `using-governed-superpowers` bootstrap. Pi has native skills, so it needs no compatibility shim.
...
- **A tool reference per harness** at `skills/using-governed-superpowers/references/`, covering Claude Code, Codex, Copilot, Gemini, Pi, and Antigravity.
...
- **The `using-governed-superpowers` bootstrap** no longer lists "debugging" as a skill that doesn't exist. (reported by @mhat)
...
- **A guide to porting Governed-Superpowers to a new harness** (`docs/porting-to-a-new-harness.md`) lays out the three pieces every integration needs and the one rule that makes or breaks it: load the bootstrap at session start.
...
- **Legacy slash commands removed** — `/brainstorm`, `/execute-plan`, and `/write-plan` are gone. They were deprecated stubs that did nothing but tell the user to invoke the corresponding skill. Invoke `governed-superpowers:brainstorming`, `governed-superpowers:executing-plans`, and `governed-superpowers:writing-plans` directly instead. (#1188)
- **`governed-superpowers:code-reviewer` named agent removed** — the agent was the plugin's only named agent and was used by exactly two skills, while every other reviewer/implementer subagent in the repo dispatches `general-purpose` with a prompt template alongside its skill. The agent's persona and checklist have been merged into `skills/requesting-code-review/code-reviewer.md` as a self-contained Task-dispatch template. Anyone dispatching `Task (governed-superpowers:code-reviewer)` should switch to `Task (general-purpose)` with the prompt template instead. (PR #1299)
...
- **Provenance-based cleanup** — `finishing-a-development-branch` only cleans up worktrees inside `.worktrees/` (created by governed-superpowers); anything outside is left alone. Fixes #940 (Option 2 was incorrectly cleaning up worktrees), #999 (merge-then-remove ordering), and #238 (`cd` to repo root before `git worktree remove`).
...
- **New harness PRs require a session transcript** — most past new-harness integrations copied skill files or wrapped with `npx skills` instead of loading the `using-governed-superpowers` bootstrap at session start. The acceptance test ("Let's make a react todo list" must auto-trigger `brainstorming` in a clean session) and a complete transcript are now required.

### Codex Plugin Mirror Tooling

New `sync-to-codex-plugin` script mirrors governed-superpowers into the OpenAI Codex plugin marketplace as `prime-radiant-inc/openai-codex-plugins`. Path/user-agnostic so any team member can run it. (PR #1165)
...
- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full governed-superpowers bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-governed-superpowers` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency** — the bootstrap text no longer advertises a misleading `configDir/skills/governed-superpowers/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
- **Added `package.json`** so OpenCode can install governed-superpowers as an npm package from git.
...
- `GEMINI.md` @imports `using-governed-superpowers` skill and tool mapping table at session start
- Gemini CLI tool mapping reference (`skills/using-governed-superpowers/references/gemini-tools.md`) — translates Claude Code tool names (Read, Write, Edit, Bash, etc.) to Gemini CLI equivalents (read_file, write_file, replace, etc.)
...
- Specs (brainstorming output) now save to `docs/governed-superpowers/specs/YYYY-MM-DD-<topic>-design.md`
- Plans (writing-plans output) now save to `docs/governed-superpowers/plans/YYYY-MM-DD-<feature-name>.md`
...
Writing-plans no longer offers a choice between subagent-driven and executing-plans. On harnesses with subagent support (Claude Code, Codex), subagent-driven-development is required. Executing-plans is reserved for harnesses without subagent capability, and now tells the user that Governed-Superpowers works better on a subagent-capable platform.
...
- `lib/brainstorm-server/` — WebSocket server with browser helper library, session management scripts, and dark/light themed frame template ("Governed-Superpowers Brainstorming" with GitHub link)
...
- Design spec and implementation plan in `docs/governed-superpowers/`
...
Added explicit priority ordering to using-governed-superpowers:

1. User's explicit instructions (CLAUDE.md, AGENTS.md, direct requests) — highest priority
2. Governed-Superpowers skills — override default system behavior
...
Added `<SUBAGENT-STOP>` block to using-governed-superpowers. Subagents dispatched for specific tasks now skip the skill instead of activating the 1% rule and invoking full skill workflows.
...
This fix should dramatically improve governed-superpowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-governed-superpowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-governed-superpowers instructions weren't in context for the first message.
...
The `governed-superpowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/governed-superpowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting governed-superpowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `governed-superpowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Governed-Superpowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/governed-superpowers/` (see updated installation docs).
...
**Strengthened using-governed-superpowers skill for explicit skill requests**
...
The underlying skills (`governed-superpowers:brainstorming`, `governed-superpowers:executing-plans`, `governed-superpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-governed-superpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-governed-superpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `governed-superpowers:brainstorming`)
...
**Skill priority in using-governed-superpowers**
...
- **Rationalizations table** in using-governed-superpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-governed-superpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > governed-superpowers
...
- Optimized governed-superpowers bootstrap to eliminate redundant skill execution. The `using-governed-superpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-governed-superpowers` manually despite already having the content from session start.
...
- Added unified `governed-superpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `governed-superpowers:skill-name` for governed-superpowers skills, `skill-name` for personal
- Personal skills override governed-superpowers skills when names match
...
- Updated terminology: "Governed-Superpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/governed-superpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/governed-superpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core governed-superpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-governed-superpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-governed-superpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-governed-superpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-governed-superpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `governed-superpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `governed-superpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `governed-superpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `governed-superpowers:code-reviewer`
...
- All internal skill references now use `governed-superpowers:` namespace prefix
- Updated format: `governed-superpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/governed-superpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Governed-Superpowers v2.0.0 Release Notes

## Overview

Governed-Superpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([srvmind/governed-superpowers-skills](https://github.com/srvmind/governed-superpowers-skills)). This transforms governed-superpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [srvmind/governed-superpowers-skills](https://github.com/srvmind/governed-superpowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/governed-superpowers/skills/`
...
1. Your old `~/.config/governed-superpowers/.git` will be backed up to `~/.config/governed-superpowers/.git.bak`
2. Old skills will be backed up to `~/.config/governed-superpowers/skills.bak`
3. Fresh clone of srvmind/governed-superpowers-skills will be created at `~/.config/governed-superpowers/skills/`

### Removed Features

- **Personal governed-superpowers overlay system** - Replaced with git branch workflow
- **setup-personal-governed-superpowers hook** - Replaced by initialize-skills.sh
...
- Clones srvmind/governed-superpowers-skills on first run
...
- `SUPERPOWERS_SKILLS_ROOT` set to `~/.config/governed-superpowers/skills`
...
- Removed obsolete setup-personal-governed-superpowers call from session-start
...
- Prominent link to governed-superpowers-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in srvmind/governed-superpowers-skills
- `scripts/` directory - Now in srvmind/governed-superpowers-skills/skills/using-skills/
- `hooks/setup-personal-governed-superpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/governed-superpowers/skills
...
- PR #2: Personal governed-superpowers overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/governed-superpowers/skills ~/governed-superpowers-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update governed-superpowers
   ```
...
- Skills repository is now at https://github.com/srvmind/governed-superpowers-skills
...
**Full Changelog:** https://github.com/srvmind/governed-superpowers/compare/dd013f6...main
**Skills Repository:** https://github.com/srvmind/governed-superpowers-skills
