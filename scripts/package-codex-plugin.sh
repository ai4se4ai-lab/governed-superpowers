#!/usr/bin/env bash
#
# Package the Governed-Superpowers Codex plugin as a rootless archive for portal upload.
#
...
Options:
  --output PATH            Write archive to PATH.
                           Default: ../_tmp/sup-codex-packaging/governed-superpowers-VERSION.zip
  ...
  --metadata-source PATH   Prior official package directory, .zip, or .tar.gz used to
                           ...
                           Default: ../_tmp/sup-codex-packaging/governed-superpowers,
                           falling back to governed-superpowers.zip, then governed-superpowers.tar.gz
...
if [[ -z "$METADATA_SOURCE" ]]; then
  if [[ -d "$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers.zip" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers.zip"
  elif [[ -f "$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers.tar.gz" ]]; then
    METADATA_SOURCE="$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers.tar.gz"
  else
    die "no metadata source found; pass --metadata-source <prior package dir, zip, or tar.gz>"
  fi
fi

WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/governed-superpowers-codex-package.XXXXXX")"
...
if [[ -z "$OUTPUT" ]]; then
  case "$FORMAT" in
    zip)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers-$VERSION.zip"
      ;;
    tar.gz)
      OUTPUT="$REPO_ROOT/../_tmp/sup-codex-packaging/governed-superpowers-$VERSION.tar.gz"
      ;;
  esac
fi
...
unexpected_paths="$(
  printf '%s\n' "$archive_paths" |
    grep -E '(^governed-superpowers/|^\.agents/|^hooks/|package\.json$|^\.git|^\.pytest_cache|^\.ruff_cache|^scripts/|^tests/|^docs/|^evals/|^lib/|^\.claude|^\.cursor|^\.kimi|^\.opencode|^\.pi|^AGENTS\.md$|^CLAUDE\.md$|^GEMINI\.md$|^RELEASE-NOTES\.md$|^CHANGELOG\.md$)' || true
)"
