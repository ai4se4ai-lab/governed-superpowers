# Governed-Superpowers for Kimi Code

Complete guide for using Governed-Superpowers with [Kimi Code](https://github.com/MoonshotAI/kimi-code).
...
2. Loads `using-governed-superpowers` at session start through `sessionStart.skill`.
3. Provides Kimi-specific tool mapping through `skillInstructions`.

Kimi Code reads Governed-Superpowers skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Governed-Superpowers and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info governed-superpowers` and check diagnostics.
...
1. Confirm `/plugins info governed-superpowers` shows the plugin enabled.
