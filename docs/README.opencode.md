# Governed-Superpowers for OpenCode

Complete guide for using Governed-Superpowers with [OpenCode.ai](https://opencode.ai).
...
**Skill Priority:** Project skills > Personal skills > Governed-Superpowers skills
...
1. **Injects bootstrap context** via the `experimental.chat.messages.transform` hook, adding governed-superpowers awareness to every conversation.
2. **Registers the skills directory** via the `config` hook, so OpenCode discovers all governed-superpowers skills without symlinks or manual config.
...
1. Check OpenCode logs: `opencode run --print-logs "hello" 2>&1 | grep -i governed-superpowers`
