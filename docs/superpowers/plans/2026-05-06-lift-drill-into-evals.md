# Lift drill into governed-superpowers as `evals/` — implementation plan
...
**Spec:** `docs/governed-superpowers/specs/2026-05-06-lift-drill-into-evals-design.md` — read this first.

**Drill source location:** `/Users/jesse/Documents/GitHub/governed-superpowers/drill/` (sibling to `governed-superpowers/`).
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers
git status --short
```
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/drill
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers
test -d /Users/jesse/Documents/GitHub/governed-superpowers/drill && echo "drill source: OK"
...
rsync -a \
  ...
  --exclude='*.pyc' \
  /Users/jesse/Documents/GitHub/governed-superpowers/drill/ \
  evals/
```
...
while read -r f; do
  ...
  sha2=$(shasum -a 256 "/Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/${f#./}" | cut -d' ' -f1)
  if [ "$sha1" != "$sha2" ]; then
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals
uv sync
```
...
/Users/jesse/Documents/GitHub/governed-superpowers/drill into
/Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals.

Verify:

1. The lift commit message records the SHA reported by:
  cd /Users/jesse/Documents/GitHub/governed-superpowers/drill && git rev-parse HEAD
...
```bash
sed -n '1,20p' /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/drill/cli.py
```
...
```python
def test_set_governed-superpowers_root_default_when_unset(monkeypatch, tmp_path):
    """When SUPERPOWERS_ROOT is unset, helper sets it to PROJECT_ROOT.parent."""
    monkeypatch.delenv("SUPERPOWERS_ROOT", raising=False)
    from drill.cli import _set_governed-superpowers_root_default, PROJECT_ROOT

    _set_governed-superpowers_root_default()

    import os
    assert os.environ["SUPERPOWERS_ROOT"] == str(PROJECT_ROOT.parent)


def test_set_governed-superpowers_root_default_respects_existing(monkeypatch):
    """When SUPERPOWERS_ROOT is already set, helper does not override."""
    monkeypatch.setenv("SUPERPOWERS_ROOT", "/custom/path")
    from drill.cli import _set_governed-superpowers_root_default
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals
uv run pytest tests/test_cli.py -k set_governed-superpowers_root_default -v
```

Expected: 2 tests fail with `AttributeError: module 'drill.cli' has no attribute '_set_governed-superpowers_root_default'`.

- [ ] **Step 4: Add the helper to cli.py**

Edit `/Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/drill/cli.py`. Replace lines 1–14 with:
...
def _set_governed-superpowers_root_default() -> None:
    ...
    pointing at the governed-superpowers checkout. After lifting drill into
    governed-superpowers/evals/, the parent of PROJECT_ROOT is always the
    governed-superpowers root, so we can supply this default automatically.
...
The bottom-of-module call to `_set_governed-superpowers_root_default()` runs at import time, immediately after `load_dotenv()`. This ensures both `engine.py` and `setup.py` (which read `os.environ["SUPERPOWERS_ROOT"]` directly) and the YAML interpolation (which reads `os.environ` when the backend YAML is loaded) all see the value.
...
Adds _set_governed-superpowers_root_default() to drill/cli.py, called at
module import after load_dotenv(). PROJECT_ROOT resolves to evals/
post-lift; its parent is the governed-superpowers repo root, which is the
...
```bash
grep -A3 'required_env:' /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/backends/codex.yaml
grep -A2 'required_env:' /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/backends/gemini.yaml
```
...
```bash
cat /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/backends/codex.yaml
```
...
```python
# Before:
def test_codex_requires_governed-superpowers_root():
    backend = load_backend("codex")
    assert "SUPERPOWERS_ROOT" in backend.required_env

# After:
def test_codex_does_not_require_governed-superpowers_root():
...
```bash
export SUPERPOWERS_ROOT=/path/to/governed-superpowers
...
`SUPERPOWERS_ROOT` defaults to the parent of `evals/` (the governed-superpowers repo root) and only needs to be set if you're running drill against a different governed-superpowers checkout.
```
...
```
SUPERPOWERS_ROOT=/path/to/governed-superpowers
...
`SUPERPOWERS_ROOT` defaults to the parent of `evals/` (the governed-superpowers repo root). Override only if running drill against a different governed-superpowers checkout.
```
...
```bash
ls /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/tests/skill-triggering/prompts/
```
...
```bash
for f in /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/tests/explicit-skill-requests/*.sh /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/tests/explicit-skill-requests/prompts/*.txt; do
  echo "=== $f ==="
  cat "$f" | head -30
done
```
...
```bash
ls /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/fixtures/sdd-go-fractals/
ls /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/evals/fixtures/sdd-svelte-todo/
```
...
```bash
cat /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/tests/claude-code/test-subagent-driven-development.sh
```
...
- Annotate (do not rewrite): `RELEASE-NOTES.md`, `docs/governed-superpowers/plans/*.md`
...
| `docs/governed-superpowers/plans/*.md` | Annotate, don't rewrite (dated artifact) |
...
For each `RELEASE-NOTES.md` or `docs/governed-superpowers/plans/*.md` hit, add an inline annotation at the *first* hit per file:
...
```
Working directory: /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers
...
Search the entire governed-superpowers tree (excluding evals/, node_modules/,
...
and docs/governed-superpowers/plans/*.md are annotated as dated artifacts,
not rewritten."
```
...
Open `/Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/docs/testing.md` and replace the file content with this structure (preserve the existing Plugin-test details where applicable):

```markdown
# Testing Governed-Superpowers

Governed-Superpowers has two distinct kinds of tests, each in its own directory:
...
Open `/Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/.gitignore` and add at the bottom:
...
```bash
cd /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers/tests/brainstorm-server
...
**Branch:** f/evals-lift, in /Users/jesse/Documents/GitHub/governed-superpowers/governed-superpowers
...
gh pr create \
  ...
  --head f/evals-lift \
  --reviewer arittr \
  --title "Lift drill into governed-superpowers as evals/ harness" \
  --body "$(cat <<'EOF'
...
Yes. Cross-runtime evaluation is core to governed-superpowers, the migration to drill scenarios was already underway in this repo, and the eval harness needs to be discoverable in-tree to be findable.

## What alternatives did you consider?

- Vendored copy + sync script (drill repo continues independently). Rejected: divergence risk; single-source-of-truth wins.
- git subtree merge (preserves drill history in-tree). Rejected: governed-superpowers' git history grows by 50+ commits, the merge commit is ugly, subtrees are operationally heavy.
- Keep drill as a sibling repo and just polish docs. Rejected: doesn't solve the discoverability problem.

## Does this PR contain multiple unrelated changes?

No — every change supports "drill is now evals/ inside governed-superpowers". Multiple commits for atomicity (verbatim copy, env helper, YAML updates, docs) but one direction.
...
- Initial prompt: see linked spec (`docs/governed-superpowers/specs/2026-05-06-lift-drill-into-evals-design.md`).
