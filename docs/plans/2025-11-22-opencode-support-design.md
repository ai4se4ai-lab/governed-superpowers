...
Add full governed-superpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port governed-superpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/governed-superpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/governed-superpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/governed-superpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/governed-superpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "governed-superpowers:brainstorming")')
  }),
...
1. **Inject using-governed-superpowers content**
   - Full content of the using-governed-superpowers skill
...
```javascript
// .opencode/plugin/governed-superpowers.js
...
export const Governed-SuperpowersPlugin = async ({ client, directory, $ }) => {
  const governed-superpowersDir = path.join(process.env.HOME, '.config/opencode/governed-superpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingGoverned-Superpowers = await readSkill('using-governed-superpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingGoverned-Superpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── governed-superpowers-codex        # UPDATED: Use skills-core
│   ├── governed-superpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── governed-superpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/governed-superpowers-codex`
...
2. Update `.codex/governed-superpowers-codex` to use shared core
...
1. Create `.opencode/plugin/governed-superpowers.js`
