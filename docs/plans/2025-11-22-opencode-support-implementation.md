...
> **For agentic workers:** REQUIRED SUB-SKILL: Use governed-superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full governed-superpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/governed-superpowers-codex` (lines 40-74)
...
- Reference: `.codex/governed-superpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'governed-superpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/governed-superpowers-codex` (lines 212-280)
 ...
 * (personal skills override governed-superpowers skills).
 *
 * @param {string} skillName - Name like "governed-superpowers:brainstorming" or "my-skill"
 * @param {string} governed-superpowersDir - Path to governed-superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, governed-superpowersDir, personalDir) {
    // Strip governed-superpowers: prefix if present
    const forceGoverned-Superpowers = skillName.startsWith('governed-superpowers:');
    const actualSkillName = forceGoverned-Superpowers ? skillName.replace(/^governed-superpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly governed-superpowers:)
    if (!forceGoverned-Superpowers && personalDir) {
    ...
    // Try governed-superpowers skills
    if (governed-superpowersDir) {
        const governed-superpowersPath = path.join(governed-superpowersDir, actualSkillName);
        const governed-superpowersSkillFile = path.join(governed-superpowersPath, 'SKILL.md');
        if (fs.existsSync(governed-superpowersSkillFile)) {
            return {
                skillFile: governed-superpowersSkillFile,
                sourceType: 'governed-superpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/governed-superpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/governed-superpowers-codex` (add import at top)
...
Run: `node -c .codex/governed-superpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/governed-superpowers-codex
...
**Files:**
- Modify: `.codex/governed-superpowers-codex` (lines 40-74)
...
Run: `.codex/governed-superpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/governed-superpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/governed-superpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/governed-superpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/governed-superpowers.js`
...
/**
 * Governed-Superpowers plugin for OpenCode.ai
 *
...
const governed-superpowersSkillsDir = path.join(homeDir, '.config/opencode/governed-superpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const Governed-SuperpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/governed-superpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/governed-superpowers.js
...
**Files:**
- Modify: `.opencode/plugin/governed-superpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "governed-superpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > governed-superpowers)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            governed-superpowersSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/governed-superpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the governed-superpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const governed-superpowersSkills = skillsCore.findSkillsInDir(
            governed-superpowersSkillsDir,
          ...
          const allSkills = [...personalSkills, ...governed-superpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install governed-superpowers skills to ~/.config/opencode/governed-superpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'governed-superpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-governed-superpowers skill content
      const usingGoverned-SuperpowersPath = skillsCore.resolveSkillPath(
        'using-governed-superpowers',
        governed-superpowersSkillsDir,
        personalSkillsDir
      );

      let usingGoverned-SuperpowersContent = '';
      if (usingGoverned-SuperpowersPath) {
        const fullContent = fs.readFileSync(usingGoverned-SuperpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingGoverned-SuperpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Governed-Superpowers skills: \`governed-superpowers:skill-name\` (from ~/.config/opencode/governed-superpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/governed-superpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/governed-superpowers && git pull` to update governed-superpowers.' :
        '';
...
**Below is the full content of your 'governed-superpowers:using-governed-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingGoverned-SuperpowersContent}
...
### 1. Install Governed-Superpowers Skills

```bash
# Clone governed-superpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/governed-superpowers
git clone https://github.com/srvmind/governed-superpowers.git ~/.config/opencode/governed-superpowers
```

### 2. Install the Plugin

The plugin is included in the governed-superpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/governed-superpowers/.opencode/plugin/governed-superpowers.js`
...
ln -s ~/.config/opencode/governed-superpowers/.opencode/plugin/governed-superpowers.js .opencode/plugin/governed-superpowers.js
```
...
```
use use_skill tool with skill_name: "governed-superpowers:brainstorming"
```
...
Personal skills override governed-superpowers skills with the same name.

## Updating

```bash
cd ~/.config/opencode/governed-superpowers
git pull
```
...
1. Check plugin file exists: `ls ~/.config/opencode/governed-superpowers/.opencode/plugin/governed-superpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/governed-superpowers/skills`
...
- Documentation: https://github.com/srvmind/governed-superpowers
```
...
Governed-Superpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
**Files:**
- Test: `.codex/governed-superpowers-codex`
...
Run: `.codex/governed-superpowers-codex use-skill governed-superpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/governed-superpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/governed-superpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/governed-superpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/governed-superpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/governed-superpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/governed-superpowers.js` created with tools and hooks
