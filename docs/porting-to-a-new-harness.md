# Porting Governed-Superpowers to a New Harness

This guide explains how to add support for a new harness — an IDE, CLI, or
agent runner that isn't Claude Code — so that Governed-Superpowers skills auto-trigger
...
## Part 1 — How Governed-Superpowers works across harnesses

Governed-Superpowers is the same content everywhere. What changes per harness is the thin
...
2. **Tool mapping (per-harness).** Each harness needs the action vocabulary
   ...
   `skills/using-governed-superpowers/references/<harness>-tools.md` and/or inline in the
...
3. **Bootstrap (per-harness).** At the start of every session, the full
   `skills/using-governed-superpowers/SKILL.md` is injected into the model's context,
...
A harness can support Governed-Superpowers only if it can do all of the following. Check
...
If the only way to get Governed-Superpowers in front of the model is for your human
...
1. The `using-governed-superpowers` bootstrap loads at session start, every session, with
...
model to describe its governed-superpowers. If the bootstrap injected, it knows it has
them. (OpenCode's install doc uses `opencode run --print-logs "hello" 2>&1 |
grep -i governed-superpowers` for the same goal via a different mechanism — log-grep
...
`using-governed-superpowers` skill's own description at session start). If more than one
...
`using-governed-superpowers/SKILL.md` and prints a JSON object whose **field name and
...
- Reference: `.opencode/plugins/governed-superpowers.js` (JavaScript) and
  `.pi/extensions/governed-superpowers.ts` (TypeScript). pi is the closest reference for
...
- Reference: `gemini-extension.json` (manifest, with `contextFileName`),
  ...
  reference), `skills/using-governed-superpowers/references/gemini-tools.md`.
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  - **Build/dependency check.** Decide how the harness loads your module:
    ...
    step? Governed-Superpowers is zero-runtime-dependency. pi's `import type
...
`using-governed-superpowers` skill content (wrapped in `<EXTREMELY_IMPORTANT>` tags) plus
...
governed-superpowers… for all other skills use the Skill tool" preamble, escapes it, and
...
field/nesting, matcher strings — are the harness's contract, not Governed-Superpowers',
...
pi's just says "do not try to load using-governed-superpowers again." If your harness has
...
*not* the user's own global file) pulls in two things: the `using-governed-superpowers`
skill and the harness's tool-mapping reference. `GEMINI.md`
does this with two `@`-includes (`@./skills/using-governed-superpowers/SKILL.md` and
`@./skills/using-governed-superpowers/references/<harness>-tools.md`); the harness loads
...
- **Shape A:** put it in `skills/using-governed-superpowers/references/<harness>-tools.md`.
...
- **Shape B:** the mapping is typically inlined into the bootstrap string you
  inject (see the `toolMapping` constant in `governed-superpowers.js`). pi keeps it in
...
`using-governed-superpowers/SKILL.md` tells the model to *never read skill files manually
...
2. **Native skill *discovery* but no `Skill` tool** (pi, Antigravity): the harness
   ...
   **For the bootstrap itself, prefer a declared context file (Part 6).** If the
   ...
   carries both the `using-governed-superpowers` content and the tool mapping. That is the
   ...
   **Fallback — the surfaced skill index.** If there's no context-file field but
   ...
   is the index, and `using-governed-superpowers`'s own surfaced description can be what
   ...
   - **It bootstraps *triggering*, not the *tool mapping*.** An injector prepends
     `<harness>-tools.md` alongside `using-governed-superpowers` every session. Here nothing
...
3. **No skill system at all:** there is nothing to register, and the *only*
   ...
   what it can't find: `using-governed-superpowers/SKILL.md` does **not** enumerate the
...
# 3. Smoke check: does the model know it has governed-superpowers?
...
tmux send-keys -t port-test 'What are your governed-superpowers?'; sleep 0.4; tmux send-keys -t port-test Enter
sleep 5
...
If the smoke check shows the model *doesn't* know it has governed-superpowers, the
...
|---|---|---|
| Native plugin marketplace | Claude Code | Register in `.claude-plugin/marketplace.json`; users `/plugin install`. The external `governed-superpowers-marketplace` repo is the source of truth users install from — see the release steps in `CLAUDE.md`. |
...
- **A plugin installer may silently strip *undeclared* files — so make the
  ...
  - **Ship a context file the manifest declares.** If the harness has a
    ...
    live `using-governed-superpowers/SKILL.md` + the tool mapping (wrapped in
    ...
    `✔ context : ANTIGRAVITY.md`, and a clean session reads `using-governed-superpowers`'s
    ...
    unrecognized.
  - **Otherwise lean on the installed `using-governed-superpowers` skill itself.** If the
    harness surfaces each installed skill's name + description at session start,
    the `using-governed-superpowers` description ("Use when starting any conversation…")
...
- Fill in the PR template's **"New harness support"** section and paste the
  ...
  be closed.
- Governed-Superpowers is a zero-dependency plugin. Don't add a third-party runtime
...
| Kimi Code | `.kimi-plugin/plugin.json` | manifest `sessionStart.skill` loads `using-governed-superpowers` | inline `skillInstructions` in manifest | `tests/kimi/` | marketplace or `/plugins install` GitHub URL |
| OpenCode | `.opencode/plugins/governed-superpowers.js` (declared via root `package.json` `main`) | in-process: `config` hook registers skills dir; `experimental.chat.messages.transform` injects user message | inline in `governed-superpowers.js` | `tests/opencode/` | `opencode.json` plugin git URL |
| pi | `.pi/extensions/governed-superpowers.ts` | in-process: `resources_discover` registers skills; `context` event injects user message; lifecycle-flag + compaction-aware | `piToolMapping()` inline **and** `references/pi-tools.md` | `tests/pi/` | repo-root `package.json` fields |

## Appendix B — Gotchas that have bitten porters

- **Opt-in isn't a port.** If your human partner has to do anything per session
  to get Governed-Superpowers, the acceptance test fails. Re-read Part 2.
